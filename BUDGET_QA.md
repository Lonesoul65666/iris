...
- ✅ **Money Map v2** (`src/components/Budget/MoneyMap.tsx`, top of budget overview) — reframed per Scott's 2026-06-30 braindump into the **zero-based ACTUAL "how's the month going" frame** off the $15,800 base: everyday SPENT + investing + reserves set aside, leftover = the win ("you beat the base by $X" / "still in play" in-progress / "over base — trim to fit"). Header "How the month's going · vs your $15,800 base". Kills the bottom-up "$14,584" headline Scott distrusted. **Investing is now CONFIRMABLE** — planned $1,000 renders dashed ("planned — confirm it") + a **Confirm deposit** button; tap when Fidelity alerts → solid + "✓ confirmed", persisted per-month in the `deployConfirmations` collection (browser-verified: confirm + undo + survives hard reload, no console errors). Redline #1 (allocation→actual) RESOLVED; redline #3 (split savings from reserves) still open. Foundation for the gamified surplus-deploy + Month-in-Review.
