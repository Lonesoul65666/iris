...
export const UPDATES: UpdateEntry[] = [
  {
    ...
    notes: [
      ...
      'You can connect Robinhood (and Fidelity, Kraken, Gemini) now. There is a second button in Settings → Connectors — “Connect a brokerage” — because Plaid only offers those institutions when Iris asks for the right thing, which it wasn’t. Balances feed net worth; they have no transactions to import, so Iris won’t try.',
      'Coinbase is not available through Plaid at all — that connection runs the other way, Coinbase using Plaid to check your bank. Coinbase gets its own connector instead: paste a read-only key and Iris reads the balance.',
